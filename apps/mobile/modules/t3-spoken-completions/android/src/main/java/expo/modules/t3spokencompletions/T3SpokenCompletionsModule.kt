  ...
  override fun definition() = ModuleDefinition {
    ...
    Function("speak") { text: String ->
      ...
      if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
        ...
        focus = request; manager?.requestAudioFocus(request); engine.setAudioAttributes(attrs)
      }
      engine.setLanguage(Locale.getDefault()); engine.setSpeechRate(1f); engine.speak(text.trim(), TextToSpeech.QUEUE_FLUSH, null, "t3-completion")
    }
    Function("stop") { tts?.stop(); val manager = appContext.reactContext?.getSystemService(Context.AUDIO_SERVICE) as? AudioManager; if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) focus?.let { manager?.abandonAudioFocusRequest(it) }; focus = null }
  }
