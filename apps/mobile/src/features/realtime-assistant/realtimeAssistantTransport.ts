  ...
  return {
    async start(sessionInput: ProviderRealtimeStartInput) {
      ...
      if (typeof secret !== "string" || secret.length === 0)
        throw new Error("T3 realtime bootstrap returned no client secret.");
      const model =
        typeof secretResponse.model === "string" ? secretResponse.model : "gpt-realtime-2";
      const url =
        typeof secretResponse.websocket_url === "string"
          ...
          try {
            ...
            if (type === "session.created" || type === "session.updated") {
              ...
              if (typeof delta === "string")
                audio({ data: delta, sampleRate: SAMPLE_RATE, numChannels: 1 });
            } else if (
            ...
            fail(cause instanceof Error ? cause.message : "Invalid realtime event.");
          }
        };
        ws.onerror = () => {
          const message = "OpenAI realtime connection failed.";
          fail(message);
    ...
    onStarted(listener) {
      started = listener;
      return () => {
        if (started === listener) started = () => undefined;
      };
    },
    onItemAdded(listener) {
      itemAdded = listener;
      return () => {
        if (itemAdded === listener) itemAdded = () => undefined;
      };
    },
    onAudio(listener) {
      audio = listener;
      return () => {
        if (audio === listener) audio = () => undefined;
      };
    },
    onError(listener) {
      error = listener;
      return () => {
        if (error === listener) error = () => undefined;
      };
    },
    onClosed(listener) {
      closed = listener;
