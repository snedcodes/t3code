...
it("exports one native message timeline read-only and excludes operational history", () => {
  ...
  const schema = `CREATE TABLE effect_sql_migrations (id INTEGER PRIMARY KEY, name TEXT); INSERT INTO effect_sql_migrations VALUES (${EXPECTED_SCHEMA_MIGRATION}, 'current');
    ...
    CREATE TABLE projection_threads (thread_id TEXT PRIMARY KEY, project_id TEXT, title TEXT, branch TEXT, worktree_path TEXT, archived_at TEXT, deleted_at TEXT, created_at TEXT, updated_at TEXT);
    ...
    INSERT INTO projection_threads VALUES ('t1', 'p1', 'Fixture Agent', NULL, NULL, NULL, NULL, '2026-07-27T00:01:00Z', '2026-07-27T00:02:00Z');
